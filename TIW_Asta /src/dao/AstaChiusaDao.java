...
private Connection con;
	...
	public List<AstaChiusa> getAllAste(){
		...
		try {
			while(returnListeQuery.next()) {
				...
				AstaChiusa a = new AstaChiusa(asta_id, id_utente,null, titolo,  descrizione,  immagine, aggiudicatario,  prezzo_vendita, spedizione , data_apertura );
				listaAste.add(a);
				a = null;
			}
		} catch (SQLException e) {
	...
	public List<AstaChiusa> getAllAsteByUser (String id_utente){
		ResultSet returnListeQuery = null;
		List<AstaChiusa> listaAste = new ArrayList<AstaChiusa>();
		...
		try {
			while(returnListeQuery.next()) {
				...
				AstaChiusa a = new AstaChiusa(asta_id, id_utente, null, titolo,  descrizione,  immagine, aggiudicatario,  prezzo_vendita, spedizione ,  returnListeQuery.getTimestamp("data_apertura"));
				listaAste.add(a);
				a = null;
			}
		} catch (SQLException e) {
		...
		try (PreparedStatement pstatement = con.prepareStatement(query);) {
			...
			try (ResultSet result = pstatement.executeQuery();) {
				if (result.next()) {
					...
					asta = new AstaChiusa(  asta_id, id_utente,  null,titolo,   descrizione,  immagine, aggiudicatario,  prezzo_vendita, spedizione ,  data_apertura);
